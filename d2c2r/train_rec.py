...
import codecs
import copy
#torch.cuda.set_device(1)#chage gpu
os.environ['CUDA_LAUNCH_BLOCKING'] = '1'   #cuda
parser = argparse.ArgumentParser()
# dataset part
parser.add_argument('--dataset', type=str, default='ToysG_VideoG', help='ToysG_VideoG,Phones_Ele,Cloth_Sports,Music_Movie')#ToysG_VideoG Phones_Ele Cloth_Sports Music_Movie

# model part
parser.add_argument('--model', type=str, default="d2c2r", help="The model name.")
parser.add_argument('--feature_dim', type=int, default=128, help='Initialize network embedding dimension.') #128
...
parser.add_argument('--dropout', type=float, default=0.3, help='GNN layer dropout rate.')#0.3
parser.add_argument('--optim', choices=['sgd', 'adagrad', 'adam', 'adamax'], default='adam',
                    help='Optimizer: sgd, adagrad, adam or adamax.')
parser.add_argument('--lr', type=float, default=0.001, help='Applies to sgd and adagrad.')#0.001
parser.add_argument('--lr_decay', type=float, default=0.9, help='Learning rate decay rate.')
parser.add_argument('--weight_decay', type=float, default=5e-4, help='Weight decay (L2 loss on parameters).')
parser.add_argument('--decay_epoch', type=int, default=10, help='Decay learning rate after this epoch.')#10
parser.add_argument('--leakey', type=float, default=0.1)#0.1
...
parser.add_argument('--beta', type=float, default=1.5)#1 klloss user 1.5
parser.add_argument('--beta_st', type=float, default=0.9)#1 fusion graph s 1.2
parser.add_argument('--beta_mm', type=float, default=0.5)#map
parser.add_argument('--beta_cat', type=float, default=2)#map cat or add

# train part
parser.add_argument('--num_epoch', type=int, default=200, help='Number of total training epochs.')#100
parser.add_argument('--batch_size', type=int, default=512, help='Training batch size.')#1024
parser.add_argument('--user_batch_size', type=int, default=64, help='Training batch size.')#64
...
parser.add_argument('--rate', type=int, default=0.5)
...
if "d2c2r" in opt["model"]:
    filename  = opt["dataset"]
    source_graph = "../dataset/" + filename + "/train.csv"
    ...
    target_train_data = "../dataset/" + filename + "/train.csv"
...
opt["shared_user"] = min(source_test_batch.MIN_USER, target_test_batch.MIN_USER)
...
for epoch in range(1, opt['num_epoch'] + 1):
    ...
    if s_dev_score >= max(s_dev_score_history):
        print("source best!")
        s_mrr_best, s_ndcg_5_best, s_ndcg_10_best, s_hr_1_best, s_hr_5_best, s_hr_10_best = predict(source_test_batch, 1)
        print("\nsource: \t{:.6f}\t{:.4f}\t{:.4f}\t{:.6f}\t{:.4f}\t{:.4f}".format(s_mrr_best, s_ndcg_5_best, s_ndcg_10_best, s_hr_1_best, s_hr_5_best, s_hr_10_best))

    if t_dev_score >= max(t_dev_score_history):
        print("target best!")
        t_mrr_best, t_ndcg_5_best, t_ndcg_10_best, t_hr_1_best, t_hr_5_best, t_hr_10_best = predict(target_test_batch, 0)
        print("target: \t{:.6f}\t{:.4f}\t{:.4f}\t{:.6f}\t{:.4f}\t{:.4f}".format(t_mrr_best, t_ndcg_5_best, t_ndcg_10_best, t_hr_1_best, t_hr_5_best, t_hr_10_best))
    ...
    print(
        "-----------epoch {}: train_loss = {:.6f} ".format( epoch, train_loss))
    print("||||||source_mrr={:.4f},source_ndcg_5 = {:.4f},source_ndcg_10 = {:.4f} , source_hit_1 = {:.4f},source_hit_5 = {:.4f},source_hit_10 = {:.4f},".format( s_mrr_best, s_ndcg_5_best, s_ndcg_10_best, s_hr_1_best, s_hr_5_best, s_hr_10_best))
    print(
        "||||||target_mrr={:.4f},target_ndcg_5 = {:.4f},target_ndcg_10 = {:.4f} , target_hit_1 = {:.4f},target_hit_5 = {:.4f},target_hit_10 = {:.4f},".format(
        t_mrr_best, t_ndcg_5_best, t_ndcg_10_best, t_hr_1_best, t_hr_5_best, t_hr_10_best))
